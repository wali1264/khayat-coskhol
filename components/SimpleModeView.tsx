      ...
      <head>
        ...
        <style>
          * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
          ...
          @media print {
            .invoice-wrapper { max-width: none; width: 100%; padding: 5mm; }
          }
          ...
          .shop-sub { font-size: 11pt; font-weight: 700; margin-top: 6pt; }
          .section { margin-bottom: 20pt; }
          .title { font-size: 14pt; font-weight: 900; border-bottom: 1pt solid #000; padding-bottom: 4pt; margin-bottom: 10pt; }
      ...
      <body>
        <div class="invoice-wrapper">
          <div class="header">
            ...
            ${shopInfo.address ? `<div style="font-size: 9pt; margin-top: 6pt;">${shopInfo.address}</div>` : ''}
          </div>
          <div class="section">
            <div class="grid-row"><span><b>مشتری:</b> ${customer.name}</span> <span dir="ltr"><b>کد:</b> ${customer.code}</span></div>
            <div class="grid-row"><span><b>تاریخ:</b> ${order.dateCreated}</span></div>
          </div>
          <div class="section">
...
${shopInfo.phone ? `📞 تماس: ${shopInfo.phone}` : ''}`;
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-['Vazirmatn'] select-none">
              ...
              <div key={customer.id} className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden transition-all">
                ...
                {isExpanded && (
                  <div className="px-6 pb-6 space-y-5 animate-in slide-in-from-top-2 duration-300">
                    ...
                    {/* بخش جدید: نمایش یادداشت‌های دائمی مشتری */}
                    {customer.notes && (
                      <div className="bg-indigo-50/50 border border-indigo-100/50 rounded-3xl p-4 flex gap-3 animate-in fade-in duration-300">
                        <Info size={16} className="text-indigo-400 mt-0.5 shrink-0" />
                        <div className="text-sm font-bold text-slate-700 leading-relaxed break-words">
                          {customer.notes}
                        </div>
                      </div>
                    )}
      ...
      {showReportsModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[150] flex items-end sm:items-center justify-center no-print p-4">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-end sm:items-center justify-center no-print p-4">
          ...
          <div className="relative bg-white w-full max-w-lg mx-auto rounded-t-[3rem] sm:rounded-[3rem] p-8 space-y-6 animate-in slide-in-from-bottom-10 max-h-[90vh] overflow-y-auto no-scrollbar">
            ...
            <form onSubmit={(e) => {
              ...
              {/* فیلد جدید: یادداشت‌های دائمی مشتری با رشد خودکار */}
              <div className="space-y-2 pt-2 border-t border-slate-100">
                 <label className="text-[12px] font-black text-slate-700 mr-2 uppercase">جزئیات و یادداشت‌های بیشتر مشتری</label>
                 <textarea 
                   name="notes"
                   defaultValue={typeof showCustomerModal === 'object' ? (showCustomerModal as Customer).notes : ''} 
                   className="w-full px-5 py-4 bg-slate-50 rounded-2xl focus:ring-2 ring-indigo-400 outline-none font-bold text-sm transition-all resize-none overflow-hidden min-h-[56px] border border-slate-100"
                   placeholder="مثلاً: سلیقه مشتری در تنگی لباس، ایرادات اندامی خاص و..."
                   onInput={(e) => {
